...
public class TransactionsController {
	...
	private void setupFines() {
		...
		forCol.setCellValueFactory(cd -> 
		new SimpleStringProperty(cd.getValue().getResourceName()));
		...
		tableFine.getColumns().addAll(fineCol,personCol,amountCol,
				overCol,forCol,whenCol,paidCol);
		tableFine.autosize();
	...
	private void confirmPay(Fine fine) {
		...
		TextInputDialog dialog = new TextInputDialog(String.valueOf(fine.getAmount()));
		dialog.setTitle("Confirm payment");
		dialog.setHeaderText("Enter a value beween "+MIN_PAY+" and "+fine.getAmount());
		...
		if (result.isPresent()){
			...
			if(Float.valueOf(result.get()) > MIN_PAY && 
				...
				if(Payment.makePayment(fine.getUsername(),
					...
					finesSplit.getItems().remove(tableFine); //Remove the old table
	...
	private void setupTransactions() {
		...
		//create the table
		TableColumn<Payment, String> transCol = 
				new TableColumn<Payment, String>("Transaction");
		transCol.setCellValueFactory(new PropertyValueFactory<>("transactionId"));

		TableColumn<Payment, String> amountCol = 
				new TableColumn<Payment, String>("Amount");
		amountCol.setCellValueFactory(new PropertyValueFactory<>("amount"));

		TableColumn<Payment, String> whenCol = 
				new TableColumn<Payment, String>("When");
	...
	private void setupBalance() {
		...
		try {
			newLoadedPane = 
					FXMLLoader.load(getClass().getResource("/fxml/staffBalance.fxml"));
