...
public class Copy implements Comparable<Copy> {
    ...
    private static void updateDBValue(int copyID, String field, String data) {
        try {
            ...
            PreparedStatement sqlStatement = connectionToDB
                .prepareStatement("UPDATE copies " + "set " + field + " = ? WHERE copyID=?");
    ...
    public Copy(Resource resource, int copyID, User borrower, int loanDuration, Date borrowDate, Date lastRenewal,
            Date dueDate) {
    ...
    /**
     * Sets the borrow variable by inserting the values into the borrowrecords
     * table, then updates the keeper column in the copies table
     * 
     * @param firstRequest
     */
        ...
        if (user != null) {
            try {
                ...
                PreparedStatement preparedUpdateStatement = dbConnection
                    .prepareStatement("INSERT INTO borrowRecords (copyId, username, description)" + " VALUES (?,?,?)");
    ...
    public void resetDates() throws IllegalArgumentException {
        if (borrower == null) {
            ...
            throw new IllegalStateException(
                "You are trying to reset borrow," + " due and last renewal dates while this copy is still borrowed!");
        }

    }

    /**
     * Returns a string representation of this copy suitable to display to any user
     * browsing the library. For this reason, it only says the copy ID and whether
     * it is available or not and leaves the rest of the information, which should
     * not be publicly accessible.
     * 
     * @return Shortened representation of this copy suitable for public viewing.
     */
